...
namespace TecnoShopApi.Controllers
{
    ...
    {
        private VendaItemServico servico;
        ...
        public ActionResult<List<VendaItemPoco>> GetAll(int? take = null, int? skip = null)
        {
            try
            {
                List<VendaItemPoco> list = this.servico.Listar(take, skip);
                return Ok(list);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Lista a venda de item de acordo com o código informado
        /// </summary>
        /// <param name="codigo"> Informe um Código </param>
        /// <returns></returns>
        [HttpGet("{codigo:int}")]
        public ActionResult<VendaItemPoco> GetById(int codigo)
        {
            try
            {
                VendaItemPoco poco = this.servico.PesquisarPelaChave(codigo);
                return Ok(poco);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult<VendaItemPoco> Post([FromBody] VendaItemPoco poco)
        {
            try
            {
                VendaItemPoco nova = this.servico.Inserir(poco);
                return Ok(nova);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult<VendaItemPoco> Put([FromBody] VendaItemPoco poco)
        {
            try
            {
                VendaItemPoco atPoco = this.servico.Alterar(poco);
                return Ok(atPoco);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult<VendaItemPoco> DeleteById(int codigo)
        {
            try
            {
                VendaItemPoco delPoco = this.servico.Excluir(codigo);
                return Ok(delPoco);
            }
            catch (Exception ex)
            {
