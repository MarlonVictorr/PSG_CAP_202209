...
namespace TecnoShopApi.Controllers
{
    ...
    public class MarcaController : ControllerBase
    {
        ...
        public ActionResult<List<MarcaPoco>> GetAll(int? take = null, int? skip = null)
        {
            try
            {
                List<MarcaPoco> list = this.servico.Listar(take, skip);
                return Ok(list);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Lista a marca de acordo com o código informado
        /// </summary>
        /// <param name="codigo"> Informe um Código </param>
        /// <returns></returns>
        [HttpGet("{codigo:int}")]
        public ActionResult<MarcaPoco> GetById(int codigo)
        {
            try
            {
                MarcaPoco poco = this.servico.PesquisarPelaChave(codigo);
                return Ok(poco);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult<MarcaPoco> Post([FromBody] MarcaPoco poco)
        {
            try
            {
                MarcaPoco nova = this.servico.Inserir(poco);
                return Ok(nova);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult<MarcaPoco> Put([FromBody] MarcaPoco poco)
        {
            try
            {
                MarcaPoco atPoco = this.servico.Alterar(poco);
                return Ok(atPoco);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult<MarcaPoco> DeleteById(int codigo)
        {
            try
            {
                MarcaPoco delPoco = this.servico.Excluir(codigo);
                return Ok(delPoco);
            }
            catch (Exception ex)
            {
